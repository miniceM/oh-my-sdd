...
企业级 SDD 工作流插件。**支持 3 种 AI 编程工具**：Claude Code、OpenCode、通义灵码 Qoder CN。skills + HARD_RULE 安全门禁跨工具复用。

**核心能力：**
- 5 个 SDD 斜杠命令：`/sdd-spec` `/sdd-plan` `/sdd-task` `/sdd-apply` `/sdd-review`
- 企业 Agent baseline 注入到主会话 system prompt（每种工具各自的注入机制）
- 与企业统一身份认证（AIH / `iam` CLI）对接（Claude 路径需要；OpenCode/Qoder 路径无需）
- 与企业绩效管理平台（DOP）对接，上报会话/命令/代码量

## 快速开始

按你使用的工具选一条路径。`oms-install` 不传 `--tool` 时自动检测（`which claude > which opencode > which lingma`）。
...
# 2. 显式选择工具（避免自动检测落到默认 Claude）
oms-install --tool opencode

# 3. 重启 OpenCode
#    skills + rules 已加载到 ~/.config/opencode/
#    /sdd-spec <change-name>
```
...
oms-install --tool qoder          # 再装 Qoder
...
oms-uninstall --tool qoder      # 仅清 Qoder 路径
```
...
- **OpenCode**：baseline 注入到 `~/.config/opencode/AGENTS.md`（HTML 哨兵块，幂等追加/移除）。plugin 侧用 `opencode/src/plugin.ts` 编译产物 `dist/plugin.js`，Bun 加载。
- **Qoder CN**：baseline 写入 `~/.lingma/rules/oh-my-sdd.md`（Always 类型规则，IDE 启动自动加载）。hooks 合并到 `~/.lingma/settings.json` 的 `PreToolUse/PostToolUse/UserPromptSubmit/Stop` 4 个事件。
...
| 通义灵码 Qoder CN | 通义灵码 IDE（`lingma` CLI 或 `~/.lingma/` 目录存在） |
