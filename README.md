...
- 机制：wrapper 启动时通过 `--append-system-prompt-file` 参数自动注入 baseline（运行期位置：`~/.config/claude-enterprise/baseline.md`，POSIX）/ `~/AppData/Roaming/ClaudeEnterprise/baseline.md`（Windows）；构建期源在 `content/enterprise-baseline.md`
