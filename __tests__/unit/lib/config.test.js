...
test('DEFAULT_CONFIG has required keys', async () => {
  const { DEFAULT_CONFIG } = await import('../../../packages/product/lib/config.js');
...
test('loadConfig returns defaults when file missing', async (t) => {
  ...
  const { loadConfig, DEFAULT_CONFIG } = await import('../../../packages/product/lib/config.js?' + Date.now());
...
test('saveConfig writes file and loadConfig reads it back', async (t) => {
  ...
  const mod = await import('../../../packages/product/lib/config.js?' + Date.now());
