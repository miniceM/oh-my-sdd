...
async function setupConfig(t, overrides = {}) {
  ...
  const { saveConfig } = await import('../../../packages/product/lib/config.js?' + Date.now());
...
test('report POSTs event as JSON', async (t) => {
  ...
  const { report } = await import('../../../packages/product/lib/dop-client.js?' + Date.now());
...
test('reportOrEnqueue retries on transient failure then succeeds', async (t) => {
  ...
  const { reportOrEnqueue } = await import('../../../packages/product/lib/dop-client.js?' + Date.now());
...
test('reportOrEnqueue enqueues after all retries exhausted', async (t) => {
  ...
  const { readAll } = await import('../../../packages/product/lib/event-queue.js?' + Date.now());
...
test('shouldSkipTelemetry returns true when telemetry_disabled', async (t) => {
  ...
  const { shouldSkipTelemetry } = await import('../../../packages/product/lib/dop-client.js?' + Date.now());
