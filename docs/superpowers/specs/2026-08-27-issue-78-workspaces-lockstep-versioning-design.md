...
产品的 OpenCode 安装适配器和插件 lifecycle 都需要资源所有权、AGENTS 路径和卸载
规则。它们位于产品包的非公开 `lib/opencode/` 共享资源层；插件在打包前把该目录派生到
自己的 `lib/opencode/`。产品适配器只导入产品内路径，已发布产品不能依赖仓库相邻的
插件源码；插件也不得成为该共享行为的手工维护源。
...
6. 产品 OpenCode 适配器必须只使用产品内的共享资源层；OpenCode 插件的对应资源层
   必须是其打包派生副本，而非跨包相对导入。
