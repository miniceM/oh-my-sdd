...
如果浏览器无法访问该 URL（在远程/容器化环境中常见），保持回环绑定并使用 SSH 或容器端口转发：
...
使用 `--url-host` 控制返回的 URL JSON 中显示的主机名。服务只允许回环绑定，并为每次启动随机生成会话 token。首次访问会设置 HttpOnly cookie 并跳转到无 token URL。不要共享初始 `url`，也不要把它写入项目文件或公开日志。
