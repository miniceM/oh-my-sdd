...
/**
 * Run the IAM CLI with platform-specific process-tree cleanup.
 * Runtime dependencies are injectable so Windows and POSIX behavior can be
 * verified without spawning real commands or terminating real processes.
 */
export function runIam(args, {
  input,
  timeoutMs,
    ...
    const killTree = () => {
      ...
      try {
        if (isWin) {
          ...
          execFileFn('taskkill', ['/PID', child.pid, '/T', '/F'], { stdio: 'ignore' });
        } else {
          // Unix: 杀整个进程组（负 pid）。兜底直接 kill。
          try { killFn(-child.pid, 'SIGKILL'); }
