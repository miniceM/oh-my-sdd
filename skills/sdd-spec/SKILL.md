...
**前置依赖**：`iam`（必需）、`openspec`（必需）、`dop`（必需）、`gh`（推荐）。
...
- **iam 校验**：`Bash("iam auth status -json")`。未授权 → 提示 `oms-login`，**停止**
- **openspec 检查**：`Bash("openspec --version")`。未装 → **阻塞**
- **dop 检查**：`Bash("dop --version")`。未装 → 警告（用自然语言模式）
...
`Bash("dop change update <id> --status spec-in-progress")`，失败 warn。
