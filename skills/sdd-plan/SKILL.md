...
- **writing-plans 约束**（chain 时显式传）：
  ...
  - **覆盖 writing-plans 默认 `feat(scope):` 格式**——在 Skill prompt 里给反例：
    ...
    ```
  - **TDD 强制（HARD_GATE）**：每个 task 必须显式包含 RED → GREEN → REFACTOR 三阶段步骤。
    在传给 writing-plans 的 prompt 中明确要求：
    ```
    每个 task 的 step list 必须按此顺序：
      Step RED:    写最小失败测试（含测试文件路径 + 断言内容）
      Step GREEN:  写最小实现代码让测试通过
      Step REFACTOR: 重构实现，运行全套测试确认仍 GREEN
    即使 CLI 工具、配置文件、简单工具等"看似无需测试"的场景，
    也必须有最小测试文件（.test.js / test_*.py 等）。
    ```
    这是企业 baseline "禁止跳过 TDD" HARD_RULE 的强制下沉——writing-plans 默认 TDD 是"建议"，此处升级为"强制"。
    完成后进入步骤 4 验证时，必须扫一遍 tasks.md 确认每个 task 都有 RED/GREEN/REFACTOR 三阶段。
...
- ✅ **TDD 守门（步骤 4d）**：tasks.md 必须每个 task 含 RED/GREEN/REFACTOR 三阶段；缺失则自动注入
- ✅ **writing-plans 约束显式传 TDD 强制**：在委托 prompt 中把 TDD 升级为 HARD_GATE（见步骤 3）
- ❌ 禁止写到 `docs/superpowers/`
- ❌ 禁止跳过 brainstorming 的用户 approve 步骤
- ❌ 禁止改 specs/*.md / proposal.md（都是 input）
- ❌ 禁止 `git add -A`
- ❌ 禁止生成**不含 TDD 三阶段**的 tasks.md（即便 writing-plans 默认 TDD 只是"建议"）
