...
- **writing-plans 约束**（chain 时显式传）：
  ...
  - **每个 task commit message 必须用 `[<change-id>] apply: <task-id> - <subject>`**（change-id 大写原样，从 .meta.json 读）
  - **覆盖 writing-plans 默认 `feat(scope):` 格式**——在 Skill prompt 里给反例：
    ```
    ❌ feat(ard123456): add health check
    ✅ [ARD123456] apply: T1 - add health check
    ```
...
- **4c**：扫 tasks.md 所有 `git commit -m "..."` 行，发现错误格式（`feat(scope):` / `apply(scope):` / 小写 change-id）→ **立即 Edit 重写**为 `[<id>] apply: <task-id> - <subject>`
